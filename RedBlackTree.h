...
class RedBlackTree {    // based on pseudocode from both class slides and geeksforgeeks https://www.geeksforgeeks.org/insertion-in-red-black-tree/
    TreeNode* root;
public:
    ...
    void insert(string name, int numMatching) { // iterative insert
        ...
        if (root == nullptr) { // doesn't exist, add
            ...
            while (parent != nullptr) {
                if (numMatching < parent->numMatching) { // smaller than current, needs to go left
    ...
    void printInorder(TreeNode* node, int& count, vector<pair<string, int>>& top10) {
        if (node == nullptr || count == 10) {
            return;
        }
        // since printing biggest first, reverse inorder
        printInorder(node->right, count, top10);
        if (count < 10) {
        ...
        if (count < 10) {
            printInorder(node->left, count, top10);
        }
    }
